...
function getLocalDate(date, isSeconds = false, isISO = false) {
  let regexp = new RegExp(':\\d{2}$', 'gui');

  if (!isISO)
    date = isSeconds
      ? date.toLocaleString()
      : date.toLocaleString().replace(regexp, '')
...
let getWeekDay = date => {
  ...
  return days[secondDate.getDay()];
};
console.log(getWeekDay('2019-01-27')); // среда
...
let getDateAgo = (date, days) => {
  let fourthDate = new Date(date);
  fourthDate.setDate(fourthDate.getDate() - days)
  return fourthDate.toLocaleDateString();
};
